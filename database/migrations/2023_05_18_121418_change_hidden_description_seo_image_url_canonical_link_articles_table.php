...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::connection('mysql3')->table('articles', function (Blueprint $table) {
            if (Schema::connection('mysql3')->hasColumn('articles', 'hidden_description')) //check the column
            {
                $table->mediumText('hidden_description')->nullable()->change();
            }
            if (Schema::connection('mysql3')->hasColumn('articles', 'seo_image')) //check the column
            {
                $table->mediumText('seo_image')->nullable()->change();
            }
            if (Schema::connection('mysql3')->hasColumn('articles', 'canonical_link')) //check the column
            {
                $table->mediumText('canonical_link')->nullable()->change();
            }
            if (Schema::connection('mysql3')->hasColumn('articles', 'canonical_title')) //check the column
            {
                $table->mediumText('canonical_title')->nullable()->change();
            }
            if (Schema::connection('mysql3')->hasColumn('articles', 'canonical_description')) //check the column
            {
