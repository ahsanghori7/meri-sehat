        ...
        $('#yajra').DataTable({
            ...
            , columns: [{
                ...
                , {
                    data: 'id'
                    , render: function(data, type, row) {
                        let e_url = ("{{ route('drug-edit', ['id' => '-id-']) }}").replace('-id-'
                            , row.e_id);
                        let l_url = ("{{ route('drug-goto-live', ['id' => '-id-']) }}").replace('-id-'
                            , row.e_id);
                        let draft_url = ("{{ route('drug-mark-draft', ['id' => '-id-']) }}").replace('-id-', row.e_id);
                        let d_url = ("").replace('-id-', row.e_id);
                        let slugs = "{{env('APP_URL')}}" + (row.lang_id != 1 ? row.language.slug+ "drug/" + row.slug : "drug/" + row.slug);
                        let linking_page = ("{{ route('drug-linking-page', ['id' => '-id-']) }}").replace('-id-', row.e_id);
                        let btn = ``;
                        let drug_url=("{{ route('drug-DrugArchive', ['id' => '-id-']) }}").replace('-id-'
                            , row.e_id)
                        @can('drug-page-update')
                            btn += ` <a class="btn-primary btn-sm my-2 btn cursor-pointer" title="Edit this drug" href="${e_url}"> <i class="icon-edit"></i></a>`;
                        @endcan
                        @can('drug-page-page-layout')
                        ...
                        @endcan
                        @can('drug-page-draft')
                            btn += ` <a class="btn-info btn-sm my-2 btn cursor-pointer" title="Mark this disease as draft" href="${draft_url}"> <i class="icon-drafts"></i></a>`;
                        @endcan
                        @can('drug-page-goto_live')
                            btn += ` <a class="btn-success btn-sm my-2 btn cursor-pointer" title="Goto live this drug" href="${l_url}"> <i class="icon-check"></i></a>`;
                        @endcan
                        @can('drug-page-clipboard')
                        ...
                        @endcan
                        @can('drug-page-archive')
                            btn += ` <a class="btn-danger btn-sm my-2 btn cursor-pointer" title="Archive this drug" href="${drug_url}"> <i class="icon-trash"></i></a>`;
                        @endcan
