...
<div class="d-flex align-items-center justify-content-between mb-3">
    ...
    @can('doctor-management-specialties-add')
...
@can('doctor-management-specialties-list-view')
    ...
    @if($result->user->doctorSpecialities && count($result->user->doctorSpecialities) > 0)
        @foreach ($result->user->doctorSpecialities as $speciality)
            <tr>
                ...
                <td>
                    @can('doctor-management-specialties-edit')
                    ...
                    @endcan
                    @can('doctor-management-specialties-delete')                    
...
<script>
    ...
    $(document).on('click', '.specialities_edit', function(event) {
        ...
        var specialities_edit_url = '{{ route("doctor-edit-specialities", ":id") }}';
    ...
    $('#specialities_add_submit').submit(function (event) {
        event.preventDefault();
        var specialities_add_url = '{{ route("doctor-update-specialities", ":id") }}';
        specialities_add_url = specialities_add_url.replace(':id', doctor_id);
    ...
    $(document).on('click', '.specialities_delete', function(e) {
        ...
        var specialities_delete_url = '{{ route("doctor-delete-specialities", ":id") }}';
        data.append( 'recordId',  specialityId);
        specialities_delete_url = specialities_delete_url.replace(':id', doctor_id);
        swalWithBootstrapButtons.fire({
            title: 'Are you sure?',
