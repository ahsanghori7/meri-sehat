...
@push('scripts')
<script>
    var fitness_id = "{{$result->fitness_experts_id}}"
    $('.edit_sevices').click(function() {
        let url = ("{{ route('fitness-experts-update-services', ['fitness_id' => '-id-']) }}").replace('-id-', fitness_id);
        open_iframe(url);
    })

    $('.edit_specialities').click(function() {
        let url = ("{{ route('fitness-experts-update-specialities', ['fitness_id' => '-id-']) }}").replace('-id-', fitness_id);
        open_iframe(url);
    })

    $('.edit_experiences').click(function() {
        let url = ("{{ route('fitness-experts-update-experiences', ['fitness_id' => '-id-']) }}").replace('-id-', fitness_id);
        open_iframe(url);
    })

    $('.edit_educations').click(function() {
        let url = ("{{ route('fitness-experts-update-educations', ['fitness_id' => '-id-']) }}").replace('-id-', fitness_id);
        open_iframe(url);
    })
    ...
    function open_iframe(url) {
        ...
        $('#edit_fitness_details_modal').modal('show');
        $('#edit_fitness_details_modal').children().children().html(html);
    }
