...
<div class="d-flex align-items-center justify-content-between mb-3">
    ...
    @can('fitness-management-education-add')
...
@can('fitness-management-education-list-view')
<table class="table table-striped table-bordered-none" id="educations_listing">
    ...
    <tbody>
    @if($result->user->fitnessEducation && count($result->user->fitnessEducation) > 0)
        @foreach ($result->user->fitnessEducation as $education)
            <tr>
                <td>{{$education->degree}}</td>
                <td>{{$education->institute}}</td>
                <td>{{$education->year_of_completion}}</td>
                <td>
                    @can('fitness-management-education-edit')
                    ...
                    @endcan
                    @can('fitness-management-education-delete')
...
<script>
    ...
    $(document).on('click', '.educations_edit', function(event) {
        ...
        var educations_edit_url = '{{ route("fitness-experts-edit-educations", ":id") }}';
    ...
    $('#educations_add_submit').submit(function (event) {
        event.preventDefault();
        var educations_add_url = '{{ route("fitness-experts-update-educations", ":id") }}';
        educations_add_url = educations_add_url.replace(':id', fitness_experts_id);
    ...
    $(document).on('click', '.educations_delete', function(e) {
        ...
        var educations_delete_url = '{{ route("fitness-experts-delete-educations", ":id") }}';
        data.append( 'recordId',  educationId);
        educations_delete_url = educations_delete_url.replace(':id', fitness_experts_id);
        swalWithBootstrapButtons.fire({
            title: 'Are you sure?',
