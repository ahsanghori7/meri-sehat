...
@can('fitness-management-service-add-services')
...
@can('fitness-management-service-list-view')
<table class="table table-striped table-bordered-none" id="services_listing">
    ...
    <tbody>
    @if($result->user->fitnessServices && count($result->user->fitnessServices) > 0)
        @foreach ($result->user->fitnessServices as $service)
        <tr>
            <td>{{$service->service->name}}</td>
            <td>
                @can('fitness-management-service-edit-services')
                ...
                @endcan
                @can('fitness-management-service-delete-services')
...
<script>
    ...
    $(document).on('click', '.services_edit', function(event) {
        ...
        var services_edit_url = '{{ route("fitness-experts-edit-services", ":id") }}';
    ...
    $('#services_add_submit').submit(function (event) {
        ...
        var services_add_url = '{{ route("fitness-experts-update-services", ":id") }}';
        services_add_url = services_add_url.replace(':id', fitness_experts_id);
    ...
    $(document).on('click', '.services_delete', function(e) {
        ...
        var services_delete_url = '{{ route("fitness-experts-delete-services", ":id") }}';
        data.append( 'recordId',  serviceId);
        services_delete_url = services_delete_url.replace(':id', fitness_experts_id);
        swalWithBootstrapButtons.fire({
            title: 'Are you sure?',
