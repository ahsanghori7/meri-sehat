...
{
    public $folder_name = 'sehat-a-z'; // For view routes and file calling and saving
    public $module_name = 'Sehat A-Z'; // For toast And page header
    ...
    public function index(Request $request, $reference_id){
        if($request->isMethod('post')){
            ...
            if (Gate::check($this->module_slug.'-widgets-sehat-a-z-status')) {
                $data['include_status_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-sehat-a-z-mobile_show')) {
                $data['include_is_mobile_show_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-sehat-a-z-web_show')) {
