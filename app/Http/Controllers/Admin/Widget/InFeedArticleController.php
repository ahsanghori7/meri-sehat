...
use App\Models\{ReferenceWidget, WidgetInFeedArticle as MainModel};
use App\Http\Common\Helper;
use Illuminate\Support\Facades\Validator;

class InFeedArticleController extends Controller
{
    public $folder_name = 'in-feed-article'; // For view routes and file calling and saving
    public $module_name = 'In Feed Article'; // For toast And page header
    ...
    public function __construct(array $attributes = array())
    {
        $this->middleware(function ($request, $next) {
            ...
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-title')) {
            ...
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-heading')) {
                $input_element = array_push($input_elements, [
                    "element_type" => "input",
                    "input_type" => "text",
                    "label" => "Heading",
            ...
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-description')) {
                $input_element = array_push($input_elements, [
                    "element_type" => "input",
                    "input_type" => "text",
                    "label" => "Description",
            ...
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-image')) {
                $input_element = array_push($input_elements, [
                    "element_type" => "image",
                    "label" => "Image",
                    "name" => "image",
    ...
    public function index(Request $request,$reference_id){
        if($request->isMethod('post')){
            ...
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-status')) {
                $data['include_status_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-mobile_show')) {
                $data['include_is_mobile_show_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-in-feed-article-web_show')) {
    ...
    public function form(Request $request,$reference_id){
        ...
        if (!(MainModel::where(['reference_id' => $reference_id])->first()) && $validator->fails()) {
            Helper::toast('error', 'Image required.');
