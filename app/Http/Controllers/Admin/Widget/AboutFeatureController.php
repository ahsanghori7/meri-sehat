...
{
    public $folder_name = 'about-feature'; // For view routes and file calling and saving
    public $module_name = 'About Feature'; // For toast And page header
    ...
    public function index(Request $request, $reference_id){
        if($request->isMethod('post')){
            ...
            if (Gate::check($this->module_slug.'-widgets-about-feature-status')) {
                $data['include_status_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-about-feature-mobile_show')) {
                $data['include_is_mobile_show_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-about-feature-web_show')) {
