...
class ArticleHelpfulController extends Controller
{
    public $folder_name = 'article-helpful'; // For view routes and file calling and saving
    public $module_name = 'Article Helpful'; // For toast And page header
    ...
    public function index(Request $request, $reference_id){
        if($request->isMethod('post')){
            ...
            if (Gate::check($this->module_slug.'-widgets-article-helpful-status')) {
                $data['include_status_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-article-helpful-mobile_show')) {
                $data['include_is_mobile_show_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-article-helpful-web_show')) {
