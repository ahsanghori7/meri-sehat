...
class SubcsriptionController extends Controller
{
    ...
    public function index(Request $request, $reference_id){
        if($request->isMethod('post')){
            ...
            if (Gate::check($this->module_slug.'-widgets-subscription-status')) {
                $data['include_status_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-subscription-mobile_show')) {
                $data['include_is_mobile_show_radio'] = 1;
            }
            if (Gate::check($this->module_slug.'-widgets-subscription-web_show')) {
