...
use App\Models\{Settings, Footer as MainModel};
use App\Http\Common\Helper;

class FooterMenuController extends Controller
{
    public $folder_name = 'footer'; // For view routes and file calling and saving
    public $module_name = 'Footer Menu'; // For toast And page header

    public function view(Request $request, $lang_id){
        if (Gate::denies('footer-management-view')) {
            abort(403);
        }
        ...
        if($request->isMethod('post')){
            ...
            $data['result'] = MainModel::where('lang_id',$lang_id)->orderBy('sequence','ASC')->get();
    ...
    public function add(Request $request, $lang_id){
        if (Gate::denies('footer-management-add')) {
            abort(403);
        }
        $lang_id = decrypt($lang_id);
        if($request->isMethod('post')){
    ...
    public function edit(Request $request, $id, $lang_id){
        if (Gate::denies('footer-management-update')) {
            abort(403);
        }
        $lang_id = decrypt($lang_id);
        $id = decrypt($id);
    ...
    public function update_sequence(Request $request){
        foreach($request->sequence as $key => $sequence){
            MainModel::where('id' , $sequence)->update([
                'sequence' => ($key +1) *10
            ]);
        }
        return back();
    ...
    public function getParentsByType(Request $request){
        ...
        try{
            if($type == 'nav-link-with-dropdown'){
                ...
                $data['parents'] = MainModel::whereIn('type',['dropdown-header','nav-link-with-dropdown'])->where('lang_id',$lang_id)->get();
            }
